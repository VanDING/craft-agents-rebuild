/**
 * TrajectoryTimeline — Chrome-Network-style overview strip with drag-range
 * selection, wheel zoom, right-drag panning, and rich tooltips. Ported from
 * the VanDSH timeline over the Craft trajectory model.
 */
...
function centeredRange(
  ...
  const rawStart = center - clampedWidth / 2
  const start = Math.min(Math.max(rawStart, minimum), maximum - clampedWidth)
...
function rangeFraction(
  ...
  const start = clampFraction((range.start - domainStart) / domainDuration)
  const end = clampFraction((range.end - domainStart) / domainDuration)
  return {
    start: Math.min(Math.max(start, (modelStart - domainStart) / domainDuration), 1),
    end: Math.max(Math.min(end, (modelEnd - domainStart) / domainDuration), 0),
  }
}
