 ...
 *
 * Local contract: openai-codex is OAuth-only, so the credential arrives as
 * `type: 'oauth'` (see buildPiAuthCredential in packages/shared) — exactly what
 * this init message sends.
 */
      ...
      [
        {
          ...
          providerType: 'pi',
          authType: 'oauth',
          piAuth: { provider: 'openai-codex', credential: { type: 'oauth', access: 'fake-not-a-jwt', refresh: '', expires: Date.now() + 3_600_000 } },
        },
