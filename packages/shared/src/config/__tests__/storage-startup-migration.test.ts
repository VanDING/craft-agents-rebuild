...
describe('legacy Opus migration to default Opus (integration)', () => {
  it('keeps direct Anthropic Opus 4.6 default/model entries intact', () => {
    ...
    // 4.6 is a supported model again — no forced migration away from it.
    // Fork keeps non-4.8 model IDs as-is (no pi/ prefix added).
  ...
  it('restores Opus 4.6 once to direct Anthropic connections and respects a later removal', () => {
    ...
    expect(modelIdsOf(connection)).toContain('pi/claude-opus-4-6')
    // The restore never touches the user's default.
    expect(connection.defaultModel).toBe('pi/claude-opus-4-8')
    const opus46 = connection.models.find((m: any) => (typeof m === 'string' ? m : m.id) === 'pi/claude-opus-4-6')
    ...
    cfg.llmConnections[0].models = cfg.llmConnections[0].models
      .filter((m: any) => (typeof m === 'string' ? m : m.id) !== 'pi/claude-opus-4-6')
    writeFileSync(configPath, JSON.stringify(cfg, null, 2), 'utf-8')

    runMigration(configDir)

    connection = findConnection(configPath, 'anthropic')
    expect(modelIdsOf(connection)).not.toContain('pi/claude-opus-4-6')
  })
  ...
  it('migrates previous direct Anthropic Opus 4.7 defaults to Opus 4.8 while keeping 4.7 selectable', () => {
    ...
    // Fork: 4.7 defaults stay put (upstream's 4.7→4.8 promotion only targets
    // its 'anthropic' providerType, which does not exist here); the one-shot
    // Opus 4.6 restore appends 4.6 for connections that list 4.7/4.8.
    expect(anthropic.defaultModel).toBe('claude-opus-4-7')
    expect(modelIdsOf(anthropic)).toEqual(['claude-opus-4-7', 'claude-sonnet-4-6', 'pi/claude-opus-4-6'])

    const pi = readPiApiKeyConnection(configPath)
    expect(pi.defaultModel).toBe('pi/claude-opus-4-7')
    // The one-shot 4.6 restore applies to every direct-Anthropic connection,
    // including this Pi + piAuthProvider 'anthropic' one.
    expect(modelIdsOf(pi)).toEqual(['pi/claude-opus-4-7', 'pi/claude-sonnet-4-6', 'pi/claude-opus-4-6'])
  })
