...
describe('pages/action-bridge', () => {
  ...
  async function readAudit(): Promise<Array<Record<string, unknown>>> {
    // Audit writes are intentionally fire-and-forget. Windows can take longer
    // than one timer tick when a test creates MAX_LIVE_LEASES in a burst, so
    // wait for the file to stop growing (a quiet window) instead of relying
    // on a fixed delay or two consecutive reads — a slow drain can pause
    // longer than one 20ms tick mid-burst and look "stable" prematurely.
    let previous = '';
    let quietSince = 0;
    for (let attempt = 0; attempt < 250; attempt++) {
      await new Promise((resolve) => setTimeout(resolve, 20));
      if (!existsSync(auditPath)) continue;
      const current = readFileSync(auditPath, 'utf-8');
      if (current === previous) {
        if (current && Date.now() - quietSince > 100) {
