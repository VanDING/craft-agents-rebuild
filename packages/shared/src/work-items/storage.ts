...
interface LegacyWorkItemsFileV2 {
  version: 2;
  items: WorkItem[];
  views?: unknown[];
  events: WorkItemEvent[];
...
function readFile(workspaceRootPath: string): WorkItemsFile {
  ...
  if (!existsSync(path)) return { version: WORK_ITEMS_VERSION, items: [], events: [] };

  let raw: WorkItemsFile | LegacyWorkItemsFileV1 | LegacyWorkItemsFileV2;
  try {
    raw = readJsonFileSync<WorkItemsFile | LegacyWorkItemsFileV1 | LegacyWorkItemsFileV2>(path);
  } catch (error) {
    throw new Error(`Unable to read work item store: ${path}`, { cause: error });
  }
  if (!raw || !Array.isArray(raw.items) || ![1, 2, WORK_ITEMS_VERSION].includes(raw.version)) {
    throw new Error(`Unsupported work item store format: ${path}`);
  }
  const parsed: WorkItemsFile = raw.version === 1
    ? { version: WORK_ITEMS_VERSION, items: raw.items, events: [], migrations: raw.migrations }
    : raw.version === 2
      ? { version: WORK_ITEMS_VERSION, items: raw.items, events: raw.events, migrations: raw.migrations }
      : raw;
