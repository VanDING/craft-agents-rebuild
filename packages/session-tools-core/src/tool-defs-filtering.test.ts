...
describe('session tool filtering helpers', () => {
  ...
  it('safe-mode helpers support MCP prefixing', () => {
    ...
    expect(blockedPrefixed.has('mcp__session__script_sandbox')).toBe(true);
    expect(blockedPrefixed.has('mcp__session__transform_data')).toBe(true);
    expect(blockedPrefixed.has('mcp__session__source_test')).toBe(true);
