...
export const ImageGenerateSchema = z.object({
  prompt: z.string().min(1).max(32_000).describe('Detailed description of the image to generate'),
  outputPath: z.string().optional().describe('Final deliverable path. Defaults to generated-images/<timestamp>.<format>.'),
  title: z.string().optional().describe('Human-readable Artifact Card title'),
  connectionSlug: z.string().optional().describe('OpenAI API-key connection slug. Omit to resolve from the session/config.'),
  model: z.string().optional().describe('Image model override. Defaults to gpt-image-2.'),
  size: z.string().regex(/^(auto|\d{2,4}x\d{2,4})$/).optional().describe('Image size such as 1024x1024, 1536x1024, or auto'),
...
The final sourcePath is not modified until the user accepts the ready draft. For a file already generated by an Office/image/PDF tool, pass initialPath; for text or JSON, pass initialText. Pass only one initial content field. The result includes editablePath for binary tools and a draft revision for compare-and-swap edits.`,

  artifact_apply: `Apply one strict text/JSON operation to an Artifact draft.

Pass the exact expectedRevision from the previous artifact result. Stale revisions and active user edit leases are rejected. Binary Office edits should operate on the managed editablePath returned by artifact_create, then call artifact_inspect or artifact_submit to snapshot them.`,
...
This does not write the final sourcePath. Only the user-facing Artifact Card/Workbench can accept or discard the ready revision.`,
  ...
  image_generate: `Generate one image with a configured OpenAI API-key connection and return it as a ready-for-review Artifact Card.

The generated binary is validated and kept in managed Artifact storage; the final outputPath is not written until the user accepts it. Use a descriptive prompt. Optional model, size, quality, background and outputFormat settings are passed to the image provider. Transparent output requires png or webp. This is a cost-incurring operation and is unavailable in Explore/Safe mode.`,
...
export const SESSION_TOOL_DEFS: SessionToolDef[] = [
  ...
  { name: 'transform_data', description: TOOL_DESCRIPTIONS.transform_data, inputSchema: TransformDataSchema, executionMode: 'registry', safeMode: 'block', handler: handleTransformData },
  { name: 'script_sandbox', description: TOOL_DESCRIPTIONS.script_sandbox, inputSchema: ScriptSandboxSchema, executionMode: 'registry', safeMode: 'block', handler: handleScriptSandbox },
