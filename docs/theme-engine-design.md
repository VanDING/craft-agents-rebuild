...
- 日期：2026-08-20
- 状态：方案已定稿（M0 完成，M1 schema 设计待开工）
...
保留自研，不引入主题框架；把现有"6 色 + 双锚点派生"升级为 **语义 token 分层 + 一个高阶 `--depth` 深度维度**，覆盖 **L1 色彩 / L2 形状 / L3 材质 / L4 排版 / L5 图标 / L6 密度** 六层自由度，让"换 token 换风格"成立，同时完全向后兼容现有 15 个主题。
...
- **15 个 preset 全部只写 6 色 + 5 面 + dark，没有一个扩展派生 token**；`--radius`/`--shadow-*`/`--font-*` 为 index.css 硬编码常量，主题引擎无法触及。
...
**决策：自研 token 分层扩展（方案 D）**。增量改动、兼容现有 15 主题、组件零改动（组件早已消费这 207 个 token，只需暴露 schema + 扩展 themeToCSS）。
...
- 所有新增 token **可选**；不声明时回退现有派生（color-mix/oklch 默认算式）。
- 15 个现有 preset 零影响（它们不声明新 token，自然走回退）。
- 唯一语义约束：`depth: glass` 会让表面半透明，与 `popoverSolid`（必须 100% 不透明）冲突，实现时对弹层强制实底。
...
| 里程碑 | 内容 | 工作量 | 风险 |
|---|---|---|---|
| M0 | 冻结覆盖 token 清单（六层 + depth 枚举） | — | 已定稿 |
| M1 | `ThemeFile` schema 扩展：中间层 token + `--depth` 派生组 + 默认派生值 | 低 | 低 |
| M2 | 扩展 `themeToCSS`：输出中间层 token + depth 展开 | 低-中 | 中（派生合并逻辑要稳） |
| M3 | 改造 1–2 个代表性 preset 示范真·风格差异 | 低 | 低 |
| M4 | 设置 UI 暴露可调项 | 中 | 低 |
| M5 | 重写 index.css 硬编码常量（radius/shadow/font 纳入主题） | 中-高 | 中（引用面广，需回归） |

**实施顺序原则**：P0 = 纯 schema 层（L1/L3/L4 中不碰组件的部分，低成本高收益）→ P1 = 需碰组件层（L2 边框、L5 图标、L6 密度）→ P2 = 硬编码常量重写。每批独立提交，符合"分批 commit / 不自动 push"习惯。

## 8. git 安排（待确认）

主题方案 doc + 配套演示 HTML 落库于 `docs/`，开分支分批 commit、不自动 push（沿用既有策略）。
