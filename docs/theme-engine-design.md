...
保留自研，不引入主题框架；把现有"6 色 + 双锚点派生"升级为 **语义 token 分层 + 一个高阶 `--depth` 深度维度**，覆盖 **L1 色彩 / L2 形状 / L3 材质 / L4 排版 / L5 图标 / L6 密度** 六层自由度，让"换 token 换风格"成立。产品只内置一套含 light/dark 的默认主题，其他正式主题在引擎稳定后另行设计。
...
- 产品只内置 `default.json`，同一文件包含 light 与 `dark` 覆盖。
...
**实施原则**：只接入已有全局消费点，不做全组件样式重写；固定数值的特殊组件保持原样。主题引擎完成后再单独设计正式主题。
